...
func InitDB() (*Database, error) {
	...
	_, err = db.Exec(`CREATE INDEX IF NOT EXISTS idx_created_at ON generations(created_at);`)
	if err != nil {
		log.Printf("Warning: Failed to create index on created_at: %v", err)
	}

	_, err = db.Exec(`CREATE INDEX IF NOT EXISTS idx_cleanup_after ON generations(cleanup_after);`)
	if err != nil {
		log.Printf("Warning: Failed to create index on cleanup_after: %v", err)
	}
...
// SerializeParameters converts generation parameters to a JSON string
